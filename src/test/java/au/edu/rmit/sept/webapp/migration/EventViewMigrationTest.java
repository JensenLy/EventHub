...
public class EventViewMigrationTest {
  ...
  void eventsTable_should_haveNewColumns() {
    List<Map<String, Object>> columns = jdbc.queryForList("SHOW COLUMNS FROM events");
    List<String> columnNames = columns.stream().map(m -> m.get("FIELD").toString().toLowerCase()).toList();
